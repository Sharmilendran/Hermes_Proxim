...
def soundSorterMatcher():
    ...
    reduced_noise = nr.reduce_noise(y = audio_clip_band_limited, sr=sample_rate, n_std_thresh_stationary=1.5,stationary=True)
    ...
    # Set the number of channels (mono = 1, stereo = 2)
        wav_file.setnchannels(1) # Set the sample rate (same as the 'rate' variable you used)
    ...
    # creating a normal distribution from frequency intensities
    # mapping a zscore onto each intensity value
    ...
    finalDF = pd.DataFrame(araayFinal,columns=["Frame", "Count"])
    ...
    for i in range(len(finalDF)):
        ...
        ##print(f'The Values for frames were {x} before and now {y}')

    #Sound Value Extraction:

    # The needed Methods:

    for i in range(len(toConverFrames)):
        
        dflabel = pd.DataFrame(scores_np[toConverFrames[i]],columns=["Count"])
        dflabel.columns = dflabel.columns.str.strip()
       
        infered_class1 = class_names[scores_np[toConverFrames[i]].mean(axis=0).argmax()]
       
        highest_with_index = get_highest_with_index(dflabel, 3)
        for j in range(3):
                print(f"-------------------------------------------- {counter}")
                print(f'The time is {toConverFrames[i]}')
                print(classLabels[highest_with_index[j][1]])
                results.append(classLabels[highest_with_index[j][1]])
                counter = counter +1
                print(f"======================================================+{counter} ")
